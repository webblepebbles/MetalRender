...
public class MetalEntityRenderer {
  ...
  private void enqueueEntityDraws(int startVertex, CapturedEntity captured,
    ...
    if (captured.overlayVertexCount > 0 &&
  ...
  private boolean isFallingBlockEntity(Entity entity) {
    return entity != null &&
        entity.getClass().getSimpleName().equals("FallingBlockEntity");
  }

  private static boolean hasFireRenderState(EntityRenderState state) {
    return getBooleanField(state, "onFire") ||
        getBooleanField(state, "displayFireAnimation");
  }
