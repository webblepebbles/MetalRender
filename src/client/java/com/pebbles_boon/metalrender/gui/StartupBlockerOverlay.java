...
public class StartupBlockerOverlay extends Overlay {
  ...
  public void extractRenderState(GuiGraphicsExtractor ctx, int mx, int my,
    ...
    int textAvailableHeight =
        Math.max(1, Math.round(ctx.guiHeight() * TEXT_HEIGHT_FRACTION));
    ...
    int bottomTextAvailableHeight =
        Math.max(1, Math.round(ctx.guiHeight() * BOTTOM_TEXT_HEIGHT_FRACTION));
    float bottomTextScale =
        Math.min((float)stackAvailableWidth / BOTTOM_TEXT.width,
                 (float)bottomTextAvailableHeight / BOTTOM_TEXT.height);
    int bottomTextWidth =
        Math.max(1, Math.round(BOTTOM_TEXT.width * bottomTextScale));
    int bottomTextHeight =
        Math.max(1, Math.round(BOTTOM_TEXT.height * bottomTextScale));

    int footnoteAvailableHeight =
        Math.max(1, Math.round(ctx.guiHeight() * FOOTNOTE_HEIGHT_FRACTION));
    float footnoteScale =
        Math.min((float)stackAvailableWidth / FOOTNOTE.width,
                 (float)footnoteAvailableHeight / FOOTNOTE.height);
    int footnoteWidth = Math.max(1, Math.round(FOOTNOTE.width * footnoteScale));
    int footnoteHeight =
        Math.max(1, Math.round(FOOTNOTE.height * footnoteScale));

    int reservedHeight =
        textHeight + bottomTextHeight + footnoteHeight + GAP * 2 + FOOTNOTE_GAP;
