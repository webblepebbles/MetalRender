...
public class MetalWorldRenderer {
  ...
  private static final int
      FPS_PRIORITY_LOADING_BACKGROUND_SUBMISSIONS_PER_PASS = 128;
      ...
      if (jProfCount >= 120) {
        ...
        MetalLogger.info(
            "JAVA_PROFILE: texture=%.2fms prune=%.2fms build=%.2fms "
                + "lod=%.2fms (avg/%d) pending=%d queued=%d meshes=%d | lanes "
                + "builder=%d/%d instant=%d/%d interactive=%d/%d | "
                + "visible=%.2fms/%d block=%.2fms/%d tracked=%d/%d",
            textureMs, pruneMs, buildMs, lodMs, jProfCount,
  ...
  private final java.util.ArrayList<Long> sortedBuildList =
    ...
    for (int sy = 0; sy < sections.length; sy++) {
      ...
      if (maxVerticalRange != Integer.MAX_VALUE) {
        ...
        boolean withinSurfaceBand =
            highestNonAirSection != Integer.MIN_VALUE &&
  ...
  private int buildFromPendingSet(int playerChunkX, int playerSectionY,
    ...
    if (sortedListDirty) {
      ...
      final boolean startupSolidFill =
          loadingMode &&
          loadingModeMeshCount < STARTUP_SOLID_FILL_MESH_THRESHOLD;
    ...
    while (!sortedBuildList.isEmpty() && built < maxSubmit &&
      ...
      if (importantCandidate == null) {
        ...
        highPriority = true;
        remainingPrioritizedBuilds =
            Math.max(0, remainingPrioritizedBuilds - 1);
      ...
      boolean interactivePriority =
          candidate.lodLevel == 0 && highPriority &&
  ...
  public int getThermalState() { return NativeBridge.nGetThermalState(); }
